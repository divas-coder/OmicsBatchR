...
#'   Higher = better (range: 1 to n_batches).
#' - **cLISI** (Cell-type LISI): measures how well cell types are preserved.
#'   Lower = better (range: 1 to n_celltypes).
...
#' @param k Integer. Number of neighbours for LISI and kBET. Default: `30`.
#' @param verbose Logical. Default: `TRUE`.
#'
#' @return A data frame with columns: `method`, `iLISI`, `cLISI`,
#'   `kBET_acceptance`, `overall_score`.
#'
#' @examples
#' \dontrun{
...
benchmark_methods <- function(results,
  ...
  for (method in names(results)) {
    ...
    # ── iLISI ─────────────────────────────────────────────────────────────────
    ilisi <- tryCatch({
      mean(.compute_lisi(embedding, batch_vec, k = k), na.rm = TRUE)
    ...
    if (!is.null(celltype_key) && celltype_key %in% colnames(obj@meta.data)) {
      clisi <- tryCatch({
        mean(.compute_lisi(embedding, obj@meta.data[[celltype_key]], k = k),
             na.rm = TRUE)
      }, error = function(e) NA_real_)
    }
...
#'
#' @return A list with:
#' - `best_method`: name of the best method (character)
#' - `seurat_obj`: the corrected Seurat object
#' - `benchmark_scores`: full benchmark data frame
#'
#' @examples
#' \dontrun{
#' results <- run_batch_correction(obj, batch_key = "batch")
...
select_best_method <- function(results,
  ...
  scores      <- benchmark_methods(results, batch_key = batch_key,
                                   celltype_key = celltype_key, verbose = verbose)
...
# ── Internal: LISI (Local Inverse Simpson's Index) ────────────────────────────

.compute_lisi <- function(embedding, labels, k = 30) {
  n_cells <- nrow(embedding)
  labels  <- as.factor(labels)

  knn_result <- BiocNeighbors::findKNN(embedding, k = k,
                                       BNPARAM = BiocNeighbors::AnnoyParam())
  knn_idx    <- knn_result$index

  lisi_scores <- vapply(seq_len(n_cells), function(i) {
    neighbour_labels <- labels[knn_idx[i, ]]
    label_props      <- prop.table(table(neighbour_labels))
    1 / sum(label_props^2)          # Inverse Simpson's index
  }, numeric(1))
...
.compute_kbet <- function(embedding, batch_labels, k = 30) {
  ...
  test_cells <- sample(seq_len(n_cells), min(1000L, n_cells))

  accept_vec <- vapply(test_cells, function(i) {
    neighbour_labels  <- batch_labels[knn_idx[i, ]]
    observed_counts   <- table(factor(neighbour_labels, levels = levels(batch_labels)))
    chi_result        <- suppressWarnings(chisq.test(observed_counts, p = global_props))
...
# ── Internal: composite overall score ────────────────────────────────────────
...
.compute_overall_score <- function(scores_df) {
  ...
  clisi_norm <- if (all(is.na(scores_df$cLISI))) {
    rep(0.5, nrow(scores_df))
  } else {
    1 - norm01(scores_df$cLISI)   # lower cLISI = better, so invert
  }
  ...
  scores_df$overall_score <- round(
    0.40 * ilisi_norm + 0.30 * clisi_norm + 0.30 * kbet_norm, 4
  )
  return(scores_df)
}
